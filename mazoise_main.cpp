...
/*   Updated: 2022/02/15 20:29:53 by Leo Suardi       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class Awesome {
	...
	public:
		...
		Awesome( void ) : _n( 42 ) { } //should not happen too often or else there is a wrong use of allocator (which calls the copy constructor)
		Awesome( int n ) : _n( n ) { std::cout << "int constructor" << std::endl; }
		Awesome( Awesome const &rhs ) : _n( 42 ) { std::cout << "cpy constructor" << std::endl; *this = rhs;}
...
void	insert_tests()
{
	...
	print_vector<T>(test);
	std::vector<T> tmp(200, 30);
	test.insert(test.begin() + 12, tmp.begin(), tmp.end());
	print_vector<T>(test);
